...
#endif
// watch shader Mods/FickleFox/assets/shaders/polychrome.fs
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    saturated_colour = lighten(tex, saturated_colour * 5.0) / glitter_brightness; // Removing dark colors from saturated color, then making it darker
	   ...
	   // === DIAGONAL LINES SETUP ===
    float stripeFrequency = 60.0;
    float diagonalLines = sin((uv.x + uv.y) * stripeFrequency); // Diagonal line pattern
    float stripe = smoothstep(0.3, 0.5, abs(diagonalLines)); // Thinner lines

    // === CLOUD-LIKE MASK (Fake Voronoi Style) ===
    float noiseTime = time * 0.6;
    float smoke1 = sin(uv.x * 6.0 + noiseTime) * 0.7;
    float smoke2 = cos(uv.y * 9.0 - noiseTime * 1.3) * 0.5;
    float smoke3 = sin(uv.x * 2.5 + uv.y * 3.2 + noiseTime * 1.1);
    float noiseCloud = (smoke1 + smoke2 + smoke3) * 0.3 + 0.5;

    // === Reveal stripes only where cloud allows
    float etchedOverlay = stripe * smoothstep(0.5, 0.7, noiseCloud);

    // === Tint the stripes — white or gold? Your call
    vec3 foilColor = vec3(1.0); // white foil
    tex.rgb = mix(tex.rgb, foilColor, etchedOverlay * 0.25); // blend lightly
