...
//    watch shader Mods/FickleFox/assets/shaders/secretRare.fs
...
vec4 effect(vec4 color, Image tex, vec2 texture_coords, vec2 screenCoords)
{
    ...
    for (int i = -1; i <= 1; i++) {
        ...
        vec2 p = hash2(ip + neighbor);
        vec2 diff = neighbor + p - fp;
        float dist = dot(diff, diff);

        if (dist < minDist) {
            minDist = dist;
            cellHue = fract(sin(dot(ip + neighbor, vec2(419.2, 371.9))) * 43758.5453);
            point = p;
        }
    }
